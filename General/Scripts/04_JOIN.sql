...
-- 21행 조회
-- 왜? 23행이 아니라 21행일까
-- EMPLOYEE에 DEPT_CODE값이 NULL인행 2개 있지만
...
FROM EMPLOYEE EMP
JOIN
EMPLOYEE MGR ON(EMP.EMP_ID= MGR.MANAGER_ID)
...
WHERE DEPT_CODE = DEPT_ID 
AND LOCATION_ID =LOCAL_CODE; -- AND 구문 사용
...
-- EMPLOYEE와 LOCATION은 겹치는 컬럼이 없기 때문에 
-- EMPLOYEE -> DEPARTMENT -> LOCATION 순서로 조인
SELECT EMP_NAME, DEPT_TITLE , LOCAL_NAME
FROM EMPLOYEE 
-- EMPLOYEE 컬럼 14개
...
--FROM EMPLOYEE 
--JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
--> EMPLOYEE와 LOCATION과 겹치는 컬럼 X -> JOIN 불가
...
SELECT EMP_NAME ,E.BONUS ,DEPT_TITLE, L.LOCAL_NAME 
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE=J.JOB_CODE)
JOIN DEPARTMENT D ON(E.DEPT_CODE= D.DEPT_ID)
JOIN LOCATION L ON (D.LOCATION_ID = L.LOCAL_CODE)
...
SELECT E.EMP_NAME 사원명, D.DEPT_TITLE 부서명,
