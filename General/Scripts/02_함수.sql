...
-- EMPLOYEE 테이블에서
-- 사원들의 이메일 아이디 조회GH하기 (뒤에 주소 필요없음)
 ...
 * 
 * - 옵션 : LEADING(앞), TRAILER(뒤), BOTH(양쪽, 기본값)
 */
...
/* <숫자 관련 함수>
 * 
 * - MOD(숫자, 나눌 값) : RUFRHKFH SKAJWL QKSGHKS
 * - ABS(숫자) : 절대값
 * - CEIL(실수) : 올림 -> 정수 형태로 반환
 * - FLOOR(실수) : 내림 -> 정수 형태로 반환
 * 
 * - ROUND(실수 [,소수점 위치]) : 반올림
 * 1) 소수점 위치 X : 소수점 첫째자리에서 반올림 -> 정수형태
...
--5e46dd08-1c63-fb2d-86ea-9a473c5c6cc4
...
FROM DUAL; -- 5개월 (CEIL로 올림)

/* 나이 구하기 */
-- 2001.03.20생의 나이,만나이 구하기
SELECT
CEIL(
(SYSDATE - TO_DATE('1999.03.05','YYYY-MM-DD')) /365+1) AS "한국식 나이",
FLOOR(
MONTHS_BETWEEN(SYSDATE, TO_DATE('1999.03.05','YYYY-MM-DD')) /12) 
FROM DUAL;

/***********************************/
-- 정확한 날짜 계산에는 MONTH_BETWEEN이 더 좋다
...
-- 기본적인 날짜 작성 순서는
-- 포맷 지정을 안해도 해석 가능
SELECT '2025-02-20' AS 문자열
, TO_DATE('2025-02-20') AS 날짜
FROM DUAL;

-- 일반적인 날짜 패턴이 아니거나
-- 영어 외 문자 포함시 포맷 지정 필수
SELECT
...
SELECT 
100000000,
TRIM(TO_CHAR(100000000, 'L999,999,999')) -- TRIM으로 공백제거 
FROM DUAL;
...
-- 오늘 날찌를 'YYYY/MM/DD DAY' 문자열로 변경
SELECT 
CURRENT_DATE,
TO_CHAR(CURRENT_DATE, 'YYYY/MM/DD DAY') AS "오늘 날짜" 
FROM DUAL;

-- 오늘 날짜를 '2025.02.22 (목) 오후 04:27:00' 변환
SELECT TO_CHAR(SYSDATE,'YYYY.MM.DD (DY) AM HH:MI:SS') AS "오늘 날짜" 
FROM DUAL;

-- 오늘 날짜 변환
-- '2025년 02월 20일 (목) 오후 04시 30분 20초' 변환
SELECT TO_CHAR(SYSDATE, 'YYYY"년"MM"월"DD"일" (DY) AM HH"시"MI"분"SS"초"') AS "오늘 날짜"
FROM DUAL;

SELECT TO_CHAR(SYSDATE, 
'RRRR"년"MM"월"DD"일" (DY) AM HH"시"MI"분"SS"초"')
FROM DUAL;
