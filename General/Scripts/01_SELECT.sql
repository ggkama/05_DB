...
/* * DB는 날짜/시간관련 데이터를 다룰 수 있도록하는 자료형 제공
 * 
 ...
 * - CURRENT_DATE : 현재 사용자 계정의 시간 얻어오기
 * 
...
CEIL((CURRENT_DATE - HIRE_DATE)/365) -- 연차 조회 후 올림처리
FROM EMPLOYEE ;
...
-- 모든 사원의 사번, 이름, 입사일, 현재까지 근무 일수, 연차  조회
-- 단, 별칭 꼭 지정 !
SELECT EMP_ID AS 사번, EMP_NAME 이름, HIRE_DATE AS "입사한 날짜", 
FLOOR(CURRENT_DATE - HIRE_DATE) AS "근무 일수", -- 내림 처리
CEIL((CURRENT_DATE - HIRE_DATE)/365) 연차 -- 연차 조회 후 올림처리
FROM EMPLOYEE ;
...
SELECT DEPT_CODE
FROM EMPLOYEE; -- 23행 조회(중복O)

-- 사원들이 속한 부서 코드 조회
 --> 사원이 있는 부서만 조회
SELECT DISTINCT DEPT_CODE
FROM EMPLOYEE; -- 7행 조회(중복X)
...
FROM EMPLOYEE 
WHERE SALARY > 4000000 AND DEPT_CODE ='D6';


-- EMPLOYEE 테이블에서
-- 급여가 300만이상, 500만 미만인 사원의
-- 사번, 이름, 급여 조회
...
-- EMAIL 컬럼에서 @ 앞에 아이디 글자수가 5글자인 사원의
-- 사번,이름, 이메일 조회
 ...
 * 
 * [작성법]
 *  WHERE LIKE '___#_' ESCAPE '#' (원하는 특수문자로 ! # $ 모두 가능)
 * -> '#' 바로 뒤 '_'는
 *   와일드카드 X , 단순 문자열 O
 */
...
-- 사번, 이름, 급여, 부서코드를
-- 급여 내림차순으로 정렬
SELECT EMP_ID ,EMP_NAME ,SALARY ,DEPT_CODE --3
FROM EMPLOYEE  --1 
WHERE DEPT_CODE IN ('D5','D6','D9') --2
...
-- 사번, 이름, 연봉을
-- 연봉 오름차순으로 정렬
SELECT EMP_NO 사번,EMP_NAME 이름,SALARY*12 연봉
FROM EMPLOYEE 
ORDER BY 연봉 ASC; -- 별칭 이용
-- ORDER BY SALARY*12 ASC; - 연봉을 구하는 식 적용
...
-- 오름차순 기본 값 : NULLS LAST
-- 내림차순 기본 값 : NULLS FIRST
...
FROM EMPLOYEE 
ORDER BY 부서코드 ASC ,3 DESC, EMP_NAME ASC;
