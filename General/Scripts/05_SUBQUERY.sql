/*
    * SUBQUERY (서브쿼리)
	...
	-- Main Query = SELECT, INSERT, UPDATE, DELETE, CREATE
	-- Sub Query = SELECT
*/  
...
SELECT EMP_NAME ,J.JOB_NAME ,DEPT_TITLE ,SALARY
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE=J.JOB_CODE)
LEFT OUTER JOIN -- 부서가 NULL인 사람도 포함하기 위해서 OUTER조인
...
SELECT E.EMP_ID 사번, E.EMP_NAME 이름, D.DEPT_TITLE  부서
,J.JOB_NAME 직급, E.SALARY 급여 
FROM EMPLOYEE E
...
FROM EMPLOYEE 
GROUP BY DEPT_CODE
HAVING SUM(SALARY) = ( -- 서브쿼리 -> 21760000
SELECT MAX(SUM(SALARY))
FROM EMPLOYEE 
GROUP BY DEPT_CODE
);
...
SELECT E.EMP_ID,E.EMP_NAME ,NVL(D.DEPT_TITLE,'없음') AS DEPT_TITLE ,J.JOB_CODE
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (E.DEPT_CODE=D.DEPT_ID)
JOIN JOB J ON (E.JOB_CODE=J.JOB_CODE);
...
NVL(D.DEPT_TITLE,'없음') AS DEPT_TITLE ,J.JOB_CODE,
'사수' AS 구분
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (E.DEPT_CODE=D.DEPT_ID)
JOIN JOB J ON (E.JOB_CODE=J.JOB_CODE)
...
SELECT E.EMP_NO ,E.EMP_NAME ,E.DEPT_CODE ,
...
DEPT_CODE ,JOB_CODE ,EMP_NO ,HIRE_DATE 
FROM EMPLOYEE
...
/* EXISTS(서브쿼리) 
 * - 서브쿼리 조회 결과가 1행이상 존재하면 TRUE -> MAIN QUERY 결과 포함 O
 *   조회결과가 0행이면 FALSE 반환           -> MAIN QUERY 결과 포함 X
 */
...
SELECT '사수 있음'
FROM EMPLOYEE SUB
WHERE SUB.EMP_ID = MAIN.MANAGER_ID /* MAIN의 MANAGER_ID */
);
...
SELECT MAIN.EMP_ID,MAIN.EMP_NAME,NVL(D.DEPT_TITLE ,'소속없음') AS DEPT_TITLE
...
(SELECT AVG(SALARY) = 
FROM EMPLOYEE SUB
WHERE SUB.JOB_CODE = 'J6') AS "해당 직급 평균"
FROM EMPLOYEE MAIN
...
--4) 급여 내림차순으로 정렬 후 ROWNUM <= 5이하로만 조회
...
-- 1) 부서별 평균 급여 내림차순 조회
SELECT NVL(E.DEPT_CODE, '없음') AS "부서코드" ,
...
-- 2) 1번 SELECT문을 인라인뷰로 사용하여 상위 3개부서만 조회
...
-- 9. RANK() OVER / DENSE_RANK() OVER -> 서브쿼리는 아님 
-- 순위를 만들거나 조회할때 사용
...
WHERE 
	E.HIRE_DATE = (
		SELECT MIN(EE.HIRE_DATE)
		FROM EMPLOYEE EE 
		WHERE 
		NVL(EE.DEPT_CODE, '소속없음')= NVL(E.DEPT_CODE, '소속없음')
		AND 
		ENT_YN != 'Y'
) 
...
SELECT 
	...
	FLOOR(MONTHS_BETWEEN(SYSDATE, 
		TO_DATE('19' || SUBSTR(E.EMP_NO , 1, 6), 'YYYYMMDD')
	) / 12) AS "만 나이"
	,TO_CHAR(E.SALARY*(1+NVL(E.BONUS,0))* 12,'999,999,999') "연봉" 
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE=J.JOB_CODE)
WHERE 
	E.EMP_NO = ( -- TO NUMBER로 변환해서 사용 가능
		SELECT MAX(EE.EMP_NO)
		FROM EMPLOYEE EE
		WHERE EE.JOB_CODE = E.JOB_CODE
	)
ORDER BY  "만 나이" DESC;
